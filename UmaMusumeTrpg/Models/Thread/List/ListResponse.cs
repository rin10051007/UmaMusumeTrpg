...
public class ListResponseForSystemThread(List<ListItemForSystemThread> items, int length, ListSearchForThread search)
    : ListResponseForThread(items.Cast<ListItemForThread>().ToList(), length, search)
{
    public new ListSearchForThread Search { get; set; } = search;
    public new List<ListItemForSystemThread> Items { get; set; } = items;
}
